...
(defn make-generation
  ...
  (let [first-four (population-builder answer generation-size generation)]
        (let [full-gen (into[] (flatten (population-builder answer generation-size
...
(defn other-generations
  ...
  (let [generation-3 (second-population-builder second-generation generation-size generation)]
          (let [third-generation (into[] (flatten (second-population-builder second-generation generation-size
...
(defn next-generation
  [generation]
  (let [next (other-generations (:answer generation) (count generation) [] generation)]
    next))

(defn evolution
  [answer generation-size max-generations]
  (let [generation []
        prev-generation (make-generation answer generation-size generation)
        mutated-prev (map mutate-answer prev-generation)]
      (subvec (into[] (flatten (take (- max-generations 1) (iterate next-generation (map mutate-answer mutated-prev))))) 0 39)
        ))




(map penalized-score (reverse (sort-by penalized-score (evolution (make-answer knapPI_11_20_1000_100 (repeat 100 0)) 40 10))))

(evolution (make-answer knapPI_11_20_1000_100 (repeat 100 0)) 40 10)
