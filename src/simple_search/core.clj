...
(def generation-one (into [] (flatten (repeat 40 (make-answer knapPI_16_200_1000_1 (repeat 200 0))))))
(def generation-place-holder generation)


(defn next-generations
  [generation generation-size]

  (let [mutated-generation (map mutate-answer generation)
        sorted-generation (reverse (sort-by penalized-score mutated-generation))
        best (take (quot (count sorted-generation) 10) sorted-generation)
        next-gen (into [] (apply concat (repeat 10 best)))]
    (def final-gen next-gen)))
